...
void FileLogAppender::log(std::shared_ptr<Logger> logger,LogLevel::Level level, LogEvent::ptr event){
    if(level>=m_level)
    {
        m_filestraeam << m_formatter->format(logger,level,event);
}
}
...
bool FileLogAppender::reopen(){
    ...
    m_filestraeam.open(m_filename,std::ios::app);
    return !!m_filestraeam;
}
...
void LogFormatter::init(){//字符串解析部分，分成三块 1内容 2参数 3是否是命令
    ...
    for (size_t i = 0; i < m_pattern.size(); i++)
    {
        ...
        if((i+1)<m_pattern.size()){//当有两个%时 表示需要添加一个%字符后面那个%才是需要判断的
            if(m_pattern[i+1]=='%'){
                nstr.append(1,'%');
                continue;
            }
        }
        //接下来是仅有一个%号字符的情况，因此直接进行分析
        size_t n = i + 1;//n表示百分号后面的一个字符的序号，可以用来解析
        int fmt_status = 0;// 表示状态令初始状态为0
        size_t fmt_begin = 0;//设立一个初值
        
        std::string str;//内容字符串（%m中的m)
        std::string fmt;//寻找括号，观察里面的参数
        while(n<m_pattern.size()){
            ...
            if(fmt_status==0){//状态为0
                if(m_pattern[n]=='{'){
                    str = m_pattern.substr(i + 1, n - i - 1);//当检测到了{则将百分号后面的字母加入str如：%asv{}中的asv其中v=i+4
//                    std::cout << "*" << str << std::endl;
                    fmt_status = 1; //到下一个状态
                    fmt_begin = n;//记录前括号的位置
                    ++n;
                    continue;
                }
            }else if(fmt_status==1){//状态为1时寻找}
                if(m_pattern[n]=='}'){
                    fmt = m_pattern.substr(fmt_begin + 1, n - fmt_begin - 1);//找到以后将括号内的字符节取出来
//                    std::cout << "#" << fmt << std::endl;
                    fmt_status = 0; //找到截取字符状态为0
                    ++n;      //跳转到下一步
                    break;
                }
            }

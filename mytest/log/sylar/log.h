...
class LogEventWrap {//wrap作为临时对象，使用完后直接析构出发日志写入，由于日志本身的智能指针，如果声明再主函数里面，程序不结束就永远无法释放
public:
...
private:
    ...
    std::ofstream m_filestraeam;
};

//日志器
class Logger:public std::enable_shared_from_this<Logger>{//自己调用指针
public:
